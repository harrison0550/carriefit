...
Completing that future workout early re-anchors its mutable `scheduledDate` to the actual local completion date, records the original scheduled date additively, and pulls later incomplete sessions forward in program order across available non-Sunday training dates. Completed sessions and protected rest-day records are never moved, and `plannedDate` remains immutable. The same pure rule reconciles older linked history through an additive, idempotent migration so an update can repair the schedule without rewriting the completed workout snapshot.
