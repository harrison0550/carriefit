...
for (const [name, media, movement, collection = "original"] of guides) {
  const asset = path.join(root, "assets", "exercise-library", collection, media);
  ...
  assert(app.includes(`assets/exercise-library/${collection}/${media}`), `${name} must use its approved artwork`);
  assert(library.includes(`"${name}": carriefitIllustration({`), `${name} must have app-created media metadata`);
  assert(library.toLowerCase().includes(movement), `${name} metadata must identify the illustrated movement`);
  assert(serviceWorker.includes(`./assets/exercise-library/${collection}/${media}`), `${name} artwork must be available offline`);
}
